...
<%@ page import="com.dragon.crm.settings.domain.DicValue" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.dragon.crm.workbench.domain.Tran" %>
         ...
         function changeStage(stage, i) {
             ...
             $.ajax({
                 ...
                 success: function (data) {
                      ...
                      */
                     if (data.success) {
                         // 改变阶段成功后，需要在详细信息页上局部刷新 刷新阶段，可能性，修改人，修改时间，阶段历史
                         $("#stage").html(data.tran.stage);
                         $("#possibility").html(data.tran.possibility);
                         $("#editBy").html(data.tran.createBy);
                         $("#editTime").html(data.tran.createTime);

                         // 改变阶段成功后
                         // 将所有的阶段图标重新进行判断，重新赋予样式及颜色
                         changeIcon(stage, i)
